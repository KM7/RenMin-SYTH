...
function grain(p,buffer,positionx,positiony,attack,release,spread,pan){
	...
	this.randomoffset = 0; //in seconds
	///envelope
	this.source.start(this.now,this.offset + this.randomoffset,this.attack + this.release+3000); //parameters (when,offset,duration)
...
function simplePlay(p,buffer,positionx,positiony,attack,release,spread,pan){
...
function pitchShifting(){
           var tempNoteDifference=targetNote-rootNote;
			trans =Math.pow(trans*Math.pow(2,1/12),tempNoteDifference);
}
